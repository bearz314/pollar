...
# * room:{room}:current_poll:token - Random token generated when loading question bank
...
# *
# * room:{room}:poll_tokens -> hash
# * * {poll_id} - Random token generated when loading question bank
# *
...
def load_question_bank(room:str) -> None:
    ...
    for poll_id, poll_dict in polls.items():
        ...
        # Generate poll token and prepare memory for votes
        reset_poll(room=room, poll_id=poll_id, options_len=len(poll_dict.get('options')))

'''
Reset poll response
'''
def reset_poll(room:str, poll_id:str, options_len:int=None) -> None:
    #TODO make sure room exists
    # Regenerate poll token to invalidate cache
    r.hset(f'room:{room}:poll_tokens', poll_id, random.randint(100000, 999999))

    if options_len is None:
        poll = get_poll(room=room, poll_id=poll_id)
        options_len=len(poll.get('options'))
...
def delete_room_keys(room:str) -> None:
    ...
    r.delete(f'room:{room}:poll_tokens')
...
Returns success bool.
'''
def set_current_poll_state(room:str, poll_id:str) -> bool:
    # Check poll_id exists
    serialised_poll = r.hget(f'room:{room}:polls', poll_id)
    poll_token = r.hget(f'room:{room}:poll_tokens', poll_id)
    if serialised_poll is None or poll_token is None:
        return False
